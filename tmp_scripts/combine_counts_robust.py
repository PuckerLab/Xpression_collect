...
def read_tpm_file(filename):
	"""Read COUNTS file into a dictionary with gene names as keys"""
...
def main():
	...
	parser = argparse.ArgumentParser(
		description='Combine multiple COUNT files into a single file',
...
Examples:
  ...
  %(prog)s --input ./data --output ./results
	...
	parser.add_argument('-i', '--input', required=True,
						help='Path to folder containing Counts.txt files')
	...
	search_pattern = os.path.join(args.input, '*Counts*.txt')
	tpm_files = sorted(glob.glob(search_pattern))

	if not tpm_files:
		print(f"Error: No count files found in '{args.input}'!")
	...
	output_file = os.path.join(args.output, 'combined_Counts.tsv')
