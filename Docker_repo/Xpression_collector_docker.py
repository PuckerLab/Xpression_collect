...
__usage__="""
			...
			--annotation_qc < yes or no for BUSCO-based QC of the PEP file
		...
		if '--sra' in arguments:
			...
			for accession in batch:
				for attempt in range(attempts):
					...
					# fasterq-dump
					cmd = f"{fasterq_dump} --split-3 --outdir {acc_dir} --temp {acc_dir} --skip-technical --threads {cores} {prefetched_file}"
